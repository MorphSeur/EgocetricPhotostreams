...
def resNet50_second_phase(weights='imagenet', img_width=224, img_height=224): #weights='../weights.resNet50.lr_0.001.phase_2.fold_10.epoch_02.tf.hdf5', img_width=224, img_height=224):# weights='imagenet', img_width=224, img_height=224):
    if weights == 'imagenet':
        base_model = ResNet50(weights=weights, include_top=False, input_shape=(img_width, img_height, 3))
        print("eeee")
    else:
...
def probabilities_plus_lstm(feature_vector_length=21, weights=None, batch_size=1, timestep=10, stateful=True):
    ...
    model.add(TimeDistributed(Dense(224, activation='softmax'), name='predictions'))
    model.summary()
